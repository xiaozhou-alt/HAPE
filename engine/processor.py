...
def do_train(cfg,
    ...
    logger = logging.getLogger("HAPE.train")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam, target_view, _) in enumerate(train_loader):
            ...
            with amp.autocast(enabled=True):
                # 模型输出: (score, feat)
                score, feat = model(img, label=target, cam_label=target_cam, view_label=target_view)
                loss = loss_fn(score=score, feat=feat, target=target, target_cam=target_cam)
...
def do_inference(cfg, model, val_loader, num_query):
    device = "cuda"
    logger = logging.getLogger("HAPE.test")
...
def training_neat_eval(cfg, model, val_loader, device, evaluator, epoch, logger):
    evaluator.reset()
    model.eval()
    logger.info("~" * 50)
