...
class HeterogeneousModalityAdapter(nn.Module):
    def __init__(self, d_model: int):
        ...
        # RGB模态：大瓶颈 + 门控 + SiLU
        self.rgb_down_proj = nn.Linear(d_model, d_model // 4)
        self.rgb_gate_proj = nn.Linear(d_model, d_model // 4)
        self.rgb_up_proj = nn.Linear(d_model // 4, d_model)
        self.rgb_act = nn.SiLU()

        # NIR模态：中瓶颈 + 轻量门控 + ReLU
        self.nir_down_proj = nn.Linear(d_model, d_model // 6)
        self.nir_gate_proj = nn.Linear(d_model, d_model // 6)
        self.nir_up_proj = nn.Linear(d_model // 6, d_model)
        self.nir_act = nn.ReLU(inplace=True)

        # TIR模态：小瓶颈 + 无门控 + GELU
        self.tir_down_proj = nn.Linear(d_model, d_model // 8)
        self.tir_up_proj = nn.Linear(d_model // 8, d_model)
    ...
    def forward(self, x_patch: torch.Tensor, modality: str):
        if modality == 'rgb':
            x = self.rgb_down_proj(x_patch)
            x = self.rgb_act(self.rgb_gate_proj(x_patch)) * x
            x = self.rgb_up_proj(x)
        elif modality == 'nir':
            x = self.nir_down_proj(x_patch)
            x = self.nir_act(self.nir_gate_proj(x_patch)) * x
            x = self.nir_up_proj(x)
        elif modality == 'tir':
            x = self.tir_down_proj(x_patch)
            x = self.tir_act(x)
...
class ResidualAttentionBlockWithHMA(nn.Module):
    ...
    def adapter_ffn(self, x: torch.Tensor, modality: str):
        ...
        # 共享TCA适配CLS Token
        if modality in ['rgb', 'nir'] and self.trainable_shared_tca is not None:
